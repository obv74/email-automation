...
function DashboardContent() {
    ...
    try {
      ...
      setNotice(
        company.ai_enabled
          ...
          : "Inbox not auto-processed (AI is off). Use Extract instead."
      );
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="mb-5 rounded-lg border border-brand-200 bg-brand-50/60 px-4 py-3 text-sm text-slate-700">
        Paste email text and get job categories on{" "}
        <Link href="/extract" className="font-semibold text-brand-700 hover:text-brand-800">
          Extract
        </Link>
        — no Gmail link needed. Works on phone.
      </div>
